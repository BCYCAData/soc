...
<h2 class="h2 mb-1 text-lg font-semibold text-surface-950">
	What information would be useful to you?<span class="text-scale-3 ml-2 text-surface-500">
		...
		{#each communityExternalInformationOptions as { value, lable }}
			<div class="col-span-1 flex items-center">
				<input
					...
					value={Number(value)}
					checked={communityExternalProfile?.information_sheet_choices?.includes(Number(value))}
				/>
