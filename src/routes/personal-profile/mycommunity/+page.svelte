<script lang="ts">
	...
	let otherComments: string | null = $state(data.user_profile.other_comments);
	let stayInTouchChoices: number[] | null = $state(data.user_profile.stay_in_touch_choices);
	let userPostalAddress: UserPostalAddress | null = $state(data.user_profile.user_postal_address);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			otherComments = data.user_profile.other_comments;
			stayInTouchChoices = data.user_profile.stay_in_touch_choices;
...
<h2 class="h2 pt-2 text-center text-base font-semibold text-gray-900">
	We have determined that you are part of the <span class="text-orange-600"
		>{data.user_profile?.property_profile?.[0]?.community_areas?.[0]?.community} Community</span
