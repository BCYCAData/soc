<script lang="ts">
	...
	let communityMeetingChoices = $state(
		data.user_profile?.community_bcyca_profile?.community_meeting_choices
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			communityMeetingChoices =
				data?.user_profile?.community_bcyca_profile?.community_meeting_choices;
			otherCommunityMeeting = data?.user_profile?.community_bcyca_profile?.other_community_meeting;
		}
	}
</script>
