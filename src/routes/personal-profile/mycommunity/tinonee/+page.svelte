<script lang="ts">
	...
	let otherComments = $state(data.user_profile.community_tinonee_profile?.other_comments);
	let stayInTouchChoices = $state(
		data.user_profile.community_tinonee_profile?.stay_in_touch_choices
	);
	let userPostalAddress = $state(data.user_profile.user_postal_address);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			otherComments = data.user_profile.community_tinonee_profile?.other_comments;
			stayInTouchChoices = data.user_profile.community_tinonee_profile?.stay_in_touch_choices;
			userPostalAddress = data.user_profile.user_postal_address;
...
<svelte:head>
	<title>Profile-Community Tinonee</title>
</svelte:head>

<form
	id="profileTinoneeCommunityForm"
	onchange={() => {
		unsaved = true;
	}}
...
>
	<h1 class="mb-2 text-right text-2xl font-semibold text-gray-600">
		Stay In Touch With Your Tinonee Community
	</h1>

	<FormAlerts {unsaved} {formError} {formSuccess} errorMessage={formErrorMessage} />
	<h2 class="h2 text-lg font-semibold text-gray-900">
		How would you prefer to stay in touch with the Tinonee Community team? <br /><span
	...
	/>
	<input type="hidden" name="had_user_postal_address" value={hadUserPostalAddress} />
	<input
		type="hidden"
		name="community_tinonee_profile_id"
		value={data.communityProfiles?.community_tinonee_profile_id}
	/>
