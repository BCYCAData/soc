<script lang="ts">
	...
	let informationSheetChoices = $state(
		data.user_profile.community_mondrook_profile?.information_sheet_choices
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			informationSheetChoices =
				data.user_profile.community_mondrook_profile?.information_sheet_choices;
			otherInformationSheet = data.user_profile.community_mondrook_profile?.other_information_sheet;
		}
	}
</script>
