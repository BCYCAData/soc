<script lang="ts">
	...
	import type { UserPostalAddress } from '$lib/types';
	...
	let otherComments = $state(data.user_profile.community_external_profile?.other_comments);
	let stayInTouchChoices = $state(
		data.user_profile.community_external_profile?.stay_in_touch_choices
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			otherComments = data.user_profile.community_external_profile?.other_comments;
			stayInTouchChoices = data.user_profile.community_external_profile?.stay_in_touch_choices;
...
<svelte:head>
	<title>Profile-Community External</title>
</svelte:head>
