<script lang="ts">
	...
	let communityMeetingChoices = $state(
		data.user_profile.community_external_profile?.community_meeting_choices
	);
	let otherCommunityMeeting = $state(
		data.user_profile.community_external_profile?.other_community_meeting
	);

	const communityMeetingOptions = data.optionsData.communityExternalOptionsData?.object_names.find(
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			communityMeetingChoices =
				data.user_profile.community_external_profile?.community_meeting_choices;
			otherCommunityMeeting = data.user_profile.community_external_profile?.other_community_meeting;
		}
	}
</script>
...
<form
	...
	method="POST"
>
	<h1 class="text-right text-2xl font-semibold text-gray-900">Community Events</h1>
