<script lang="ts">
	...
	let informationSheetChoices = $state(
		data.user_profile.community_external_profile?.information_sheet_choices
	);
	let otherInformationSheet = $state(
		data.user_profile.community_external_profile?.other_information_sheet
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			informationSheetChoices =
				data.user_profile.community_external_profile?.information_sheet_choices;
			otherInformationSheet = data.user_profile.community_external_profile?.other_information_sheet;
		}
	}
</script>

<svelte:head>
	<title>Profile-Community External-Information</title>
</svelte:head>
