<script lang="ts">
	...
	let family_name = $state(data.user_profile.family_name);
	let fire_fighting_experience = $state(data.user_profile.fire_fighting_experience);
	let fire_trauma = $state(data.user_profile.fire_trauma);
	let first_name = $state(data.user_profile.first_name);
	let mobile = $state(data.user_profile.mobile);
	let plan_to_leave_before_fire = $state(data.user_profile.plan_to_leave_before_fire);
	let plan_to_leave_before_flood = $state(data.user_profile.plan_to_leave_before_flood);
	let residency_profile = $state(data.user_profile.residency_profile);
	let rfs_survival_plan = $state(data.user_profile.rfs_survival_plan);

	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			unsaved = false;
			// Reset form to initial state
			family_name = data.user_profile.family_name;
			fire_fighting_experience = data.user_profile.fire_fighting_experience;
			fire_trauma = data.user_profile.fire_trauma;
			first_name = data.user_profile.first_name;
			mobile = data.user_profile.mobile;
			plan_to_leave_before_fire = data.user_profile.plan_to_leave_before_fire;
			plan_to_leave_before_flood = data.user_profile.plan_to_leave_before_flood;
			residency_profile = data.user_profile.residency_profile;
			rfs_survival_plan = data.user_profile.rfs_survival_plan;
		}
	}
</script>
