...
export const load: PageServerLoad = async ({ locals: { supabase, getSessionAndUser } }) => {
	...
	const permissionTree = rolePermissions.reduce((acc, { permission }) => {
...
export const actions: Actions = {
	addRole: async ({ request, locals: { supabase, getSessionAndUser } }) => {
		await validateUserPermissions(supabase, getSessionAndUser, REQUIRED_PERMISSION);
	...
	deleteRole: async ({ request, locals: { supabase, getSessionAndUser } }) => {
		await validateUserPermissions(supabase, getSessionAndUser, REQUIRED_PERMISSION);
		const formData = await request.formData();
		const role = formData.get('role')?.toString();
