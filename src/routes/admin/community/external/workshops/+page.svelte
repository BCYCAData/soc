<script lang="ts">
	...
	let externalWorkshopsData = data?.externalWorkshopsData;
	...
	const workshopItems = [
		{
			...
			choice: 1,
			columns: externalWorkshopsColumns
		},
		{
			id: 'OnlineVideoCalls',
...
<svelte:head>
	<title>External Community Admin-Workshops</title>
</svelte:head>
...
<Accordion {value} spaceY="space-y-1">
			...
			{#snippet panel()}
				<div class="table-container" id={item.id}>
					<CommunityRequestsTable
						...
						tableData={externalWorkshopsData.filter((row: { workshop_choices: number[] }) =>
	...
	<Accordion.Item value="OtherWorkshopSuggestions">
		...
		{#snippet panel()}"
			<div class="table-container" id="OtherWorkshopSuggestions">
				<CommunityRequestsTable
					...
					tableData={externalWorkshopsData.filter(
	...
	<Accordion.Item value="WillRunWorkshops">
