...
export const load: PageServerLoad = async ({ locals: { supabase, user } }) => {
	...
	const personalProfileQueryData = supabase
	...
	type PersonalProfileData = QueryData<typeof personalProfileQueryData>;
	const { data, error: getPersonalProfileDataError } = await personalProfileQueryData;
	if (getPersonalProfileDataError) {
		console.log('GET data error Personal Profile:', getPersonalProfileDataError);
		let emailSubject: PostgRestErrorEmailSubject = {
			type: `GET data error :: Personal Profile.`,
			user: `User:: ${user.id}.`,
			time: `${new Date().toLocaleDateString()}  ${new Date().toLocaleTimeString()}`
		};
		sendPostgRestErrorEmail(emailSubject, getPersonalProfileDataError);
		error(400, `GET data error Personal Profile:  Error ${getPersonalProfileDataError.message}`);
	}
	if (data && data.length > 0) {
		const personalProfileData: PersonalProfileData = data;
		let {
			user_profile: [_userProfileData],
			property_agent,
			community_areas,
			...property_profile
		} = personalProfileData[0];
		const communityName =
			community_areas && community_areas[0] ? community_areas[0].community : undefined;
		let {
			id,
		...
		if (!property_agent || property_agent.length === 0) {
			property_agent = [
				{
					agent_mobile: '',
					agent_name: '',
		...
		if (!user_postal_address || user_postal_address.length === 0) {
			user_postal_address = [
				{
	...
	error(400, `GET data error Personal Profile Form:  'No data found or an error occurred.'`);
};
