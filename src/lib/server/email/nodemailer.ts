...
import type { ComponentType, SvelteComponent } from 'svelte'; // Keep both imports
...
export type PostgRestErrorEmailSubject = {
	type: string;
	user: string;
	time: string;
};
...
export function sendPostgRestErrorEmail(
	...
	const emailHtml = render({
		template: SOCDataBaseTemplate as unknown as ComponentType<SvelteComponent<EmailTemplateProps>>,
		props: {
